...
# use the custom 'nholuongut' profile
defaults write com.apple.Terminal "Default Window Settings" -string "nholuongut"
defaults write com.apple.Terminal "Startup Window Settings" -string "nholuongut"
...
# so we load from a saved Terminal -> Preferences -> Profiles -> nholuongut -> Settings -> Export file and override the $LINES and $COLUMNS
mac_terminal_settings="$(cat "$srcdir/nholuongut.terminal")"
...
defaults write com.apple.Terminal "Window Settings" -dict-add nholuongut "$mac_terminal_settings"
