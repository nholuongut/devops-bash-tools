...
docker_get_images(){
    ...
    #echo "$(dockerhub_search.py nholuongutnho -n 1000 | tail -n +2 | awk '{print $1}' | sort) $(sed 's/#.*//;/^[[:space:]]*$/d' "$bash_tools/setup/docker-images.txt" | uniq_order_preserved.pl)"
...
dockerpullgithub(){
    dockerpull1 nholuongutnho/{nagios-plugins,pytools,tools,centos-github,debian-github,ubuntu-github,alpine-github}
}
...
dockerpull(){
    ...
    [ -z "$images" ] && images="$(docker_get_images)"
    dockerpull1 "$images"
    images="$(grep -i -e nholuongutnho -e ":" <<< "$images")"
    #local images="$(grep -i -e ":" <<< "$images")"
    # now pull all tags, mine first, then official
    whendone "docker pull" # must be first arg so quoted, [l] trick not needed as grep -v grep's
    for image in $images; do
        #whendone "docker pull" # must be first arg so quoted, [l] trick not needed as grep -v grep's
        if [[ "$image" = nholuongutnho/* && ! "$image" =~ ":" ]]; then
